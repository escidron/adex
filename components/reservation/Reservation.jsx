...
export default function Reservation({ data, hasCard, setHasCard, setShowModal,setIsDone }) {
        ...
        <div className={`w-[400px] h-[450px] flex flex-col   shadow-lg rounded-lg border p-4 ${inter.className}`}>
            ...
            <div className='w-[90%] '>
                ...
                <div className='mt-2 flex justify-between items-center'>
                    ...
                    <p>{`$${data?.price ? formatNumberInput((data.price * counter - discount).toString()) : ''}`}</p>
                </div>
            </div>
            <button disabled={isPending?true:false} onClick={Booking} className={`z-10 flex item justify-center bg-black text-[#FCD33B] py-[8px] w-full px-[30px] rounded-md mt-4 font-[600] md:mt-5 ${!isPending ? 'hover:bg-[#FCD33B] hover:text-black' : ''}  text-lg lg:mt-10 `}>
                {data.is_automatic == '1' ? (
